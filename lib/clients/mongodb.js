    ...
    .then(function (db) {
      ...
      return listDatabases
        .then(function (result) {
          var dbList = result.databases.map(database => database.name);
      ...
      return Promise.all(colls.map(function (coll) {
        ...
        return coll.indexesAsync()
          .then(function (indexes) {
            ...
            var primaryKey = indexes.filter(row => row.name === '_id_');
            var secondaryIndexes = indexes.filter(row => row.name !== '_id_');
