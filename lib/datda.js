        ...
        return sourceDB.getTables()
          .then(function (tables) {
            ...
            var recursiveInsertTable = function(numberOfTables, currentTableIndex) {
              ...
              const tableMigrationConfig = Object.assign({
                rowOffset: options.rowOffset,
                requiredFields: []
              }, options.tableConfig[tableConfig.name]);
              const checkRequiredFields = row => {
                for (let field of tableMigrationConfig.requiredFields) {
                ...
                .then(function(numberOfRows) {
                  ...
                  var recursiveInsertRow = function (numberOfRows, currentRow) {
                    ...
                    return sourceDB.getRows(tableConfig, rowsToInsert, currentRow)
                      .then(function (rows) {
                        // skip rows that does not contain all required fields
                        rows = rows.filter(checkRequiredFields);
                        if (rows.length == 0) {
                          return true;
                        }
                  ...
                  return recursiveInsertRow(numberOfRows, tableMigrationConfig.rowOffset);
                })
                .then(insertionLatencyPromiseFn)
